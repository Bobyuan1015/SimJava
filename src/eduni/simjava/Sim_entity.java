...
public class Sim_entity extends Thread implements Cloneable {
   ...
   *              should be sent
   * @param object   An user-defined number representing the type of event.
  ...
  public void sim_schedule(int dest, double delay, int tag, Object data) {
    ...
    Sim_system.send(get_id(), dest, delay, tag, data);
  }
   ...
   * @param data  The data to be sent with the event.
   * @param src_id Entity ID or original source.
   */
  private void sim_schedule(int dest, double delay, int tag, Object data,
		int src_id) {
	if (!Sim_system.running()) {
	      return;
	    }
	    Sim_system.send(src_id, dest, delay, tag, data);
  }
   ...
   * @param p  The port through which to send the event
   */
  public void send_on_intact(Sim_event ev, Sim_port p) {
    sim_schedule(p.get_dest(), 0.0, ev.type(), ev.get_data(), ev.get_src());
  }
